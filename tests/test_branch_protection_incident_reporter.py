...
from backend.services.enterprise_branch_protection_incident_reporter import (
    DEFAULT_ISSUE_TYPE,
    DEFAULT_JIRA_PROJECT_KEY,
    build_branch_protection_incident_description,
    report_branch_protection_incident,
)

_GAPS = [{"gap": "no_protection", "severity": "critical", "description": "No branch protection configured — anyone with write access can force-push or delete this branch."}]


class TestBuildBranchProtectionIncidentDescription:
    def test_includes_core_fields(self):
        desc = build_branch_protection_incident_description("o", "r", "main", _GAPS, "critical")
        assert "o/r" in desc
        assert "main" in desc
        assert "critical" in desc
        assert "force-push" in desc


@pytest.mark.asyncio
class TestReportBranchProtectionIncident:
    async def test_returns_none_when_jira_not_registered(self):
        with patch("backend.connectors.registry.connector_registry.get", return_value=None):
            result = await report_branch_protection_incident("o", "r", "main", _GAPS, "critical")
    ...
    async def test_creates_issue_with_correct_fields(self, monkeypatch):
        monkeypatch.delenv("BRANCH_PROTECTION_JIRA_PROJECT_KEY", raising=False)
        monkeypatch.delenv("BRANCH_PROTECTION_JIRA_ISSUE_TYPE", raising=False)
        monkeypatch.delenv("BRANCH_PROTECTION_AUTO_FIX", raising=False)
        fake_jira = MagicMock()
    ...
    async def test_auto_fix_triggered_when_flag_on(self, monkeypatch):
        ...
        assert mock_fix.call_args.kwargs["owner"] == "o"
        assert mock_fix.call_args.kwargs["branch"] == "main"
    ...
    async def test_auto_fix_failure_does_not_break_ticket_result(self, monkeypatch):
        ...
        fake_jira.create_issue = AsyncMock(return_value={"key": "OPS-9"})
        with patch("backend.connectors.registry.connector_registry.get", return_value=fake_jira), \
             patch("backend.services.enterprise_branch_protection_fix_executor.enable_minimal_protection", new=AsyncMock(side_effect=RuntimeError("boom"))):
            result = await report_branch_protection_incident("o", "r", "main", _GAPS, "critical")
        assert result == {"key": "OPS-9"}
