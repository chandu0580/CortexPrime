...
Probes 17 subsystems concurrently and reports overall system health.

```mermaid
graph TB
    subgraph HC["HealthCenterService"]
        PROBE["Probe Manager"]
        MAP["Status Mapping\nraw → healthy/warning/critical"]
    ...
    MAP --> SNAP
    SNAP --> DB[("health_status_snapshots")]
```

### 17 Probed Subsystems
...
Snapshots are written to the `health_status_snapshots` table in PostgreSQL for historical analysis.

---

## 12. Backup & Restore

File-based backup system with integrity verification.

```mermaid
flowchart LR
    subgraph Entities["Backup Entities"]
        WD["Workflow Definitions"]
        CC["Connector Configurations"]
        MH["Mission History"]
        RH["Replay History"]
        AL["Audit Logs"]
        AM["Analytics Metadata"]
    end

    subgraph Backup["Backup Process"]
        EXP["Export to JSON"]
        HASH["SHA-256 Integrity Hash"]
        STORE["Store in BACKUP_DIR"]
    end

    subgraph Restore["Restore Process"]
        LOAD["Load JSON File"]
        VERIFY["Verify Integrity"]
        IMP["Import to Database"]
    end
...
|-----------|-------------|
| Backup | Export entities to JSON files with SHA-256 hash |
| Restore | Load and import JSON files after integrity check |
| Verify | Validate SHA-256 checksums of backup files |
| Rollback | Revert last restore operation |

### Configuration

| Parameter | Description |
|-----------|-------------|
| `BACKUP_DIR` | Configurable file storage path |
| Entities | 6 supported entity types |

---

## 13. Maintenance Mode

Controlled system lockdown for maintenance operations.

```mermaid
stateDiagram-v2
    [*] --> Active: System Start
    
    Active --> Maintenance: enable()
    Maintenance --> Active: disable()
    
    state Maintenance {
        [*] --> BlockNew: block_new_missions=true
        BlockNew --> AllowExisting: allow_existing_missions=true
    ...
    Maintenance --> AuditEvent: MaintenanceEvent
    
    state Integration {
        should_block_new_mission() --> BlockNew
        get_banner() --> Banner
    }
```

### MaintenanceState Dataclass

| Field | Type | Description |
|-------|------|-------------|
| `enabled` | bool | Maintenance mode active |
| `banner_message` | str | User-facing maintenance message |
| `block_new_missions` | bool | Prevent new mission starts |
| `allow_existing_missions` | bool | Let running missions complete |

### Integration Points

| Method | Purpose |
|--------|---------|
| `should_block_new_mission()` | Called by Mission Runtime before starting missions |
| `get_banner()` | Returns current banner message for UI display |

### Audit

All maintenance mode transitions persist a `MaintenanceEvent` for the audit trail. Enable/disable requires admin authentication.

---

## 14. Operational Reports

Scheduled report generation for operational insights.

```mermaid
flowchart TB
    subgraph Schedule["Report Scheduler"]
        D["Daily"]
        W["Weekly"]
        M["Monthly"]
    end

    subgraph Metrics["Report Metrics"]
        MC["Mission Counts"]
        SR["Success/Failure Rates"]
        CU["Connector Utilization"]
        WU["Workflow Utilization"]
        AS["Approval Statistics"]
        CM["Cost Metrics"]
    end
...
| Metric | Source |
|--------|--------|
| Mission Counts | Mission records |
| Success/Failure Rates | Mission records + replay events |
| Connector Utilization | Activity records |
| Workflow Utilization | Workflow definitions + approval records |
| Approval Statistics | Approval requests |
| Cost Metrics | LLM token usage, compute duration |

### Persistence

Reports are stored in the `operational_reports` table for historical access and export.
