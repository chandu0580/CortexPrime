...
> **Retired in Phase 10.29 (ADR-119).** The V1 organizational directory (organizations, departments, projects) was removed from the repository. The `/api/v1/admin/organizations…` endpoints this section documented never existed in the backend; the routes that did exist were mounted at `/api/api/…`, had no table on any database and no client. Governed tenancy (ADR-102/ADR-103: `cp_tenant`, `cp_tenant_membership`) is the organizational boundary the platform enforces and is unaffected. If organizational management becomes a product requirement it returns as an explicit product/architecture phase.
