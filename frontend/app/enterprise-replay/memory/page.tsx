...
import { MemoryReplayPanel } from "@/components/enterprise-replay/memory-replay"
...
export default function MemoryReplayPage() {
  ...
  const loadMemory = useEnterpriseReplayStore((s) => s.loadMemory)
            ...
            <motion.div variants={variants.fadeUp} className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  <Database className="h-6 w-6 text-[#38B88A]" />
                  <h1 className="text-[1.6rem] font-bold tracking-[-0.02em] text-[#111827]">Memory Replay</h1>
                </div>
                <p className="mt-0.5 text-[0.82rem] text-[#6B7280]">Working memory, semantic memory, episodic memory, and changes over time.</p>
              </div>
              <ExportButton executionId={executionId} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <ExecutionIdInput onLoad={(id) => loadMemory(id)} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <MemoryReplayPanel />
            </motion.div>
