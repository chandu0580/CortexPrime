...
import { ConnectorReplayPanel } from "@/components/enterprise-replay/connector-replay"
import { useEnterpriseReplayStore } from "@/store/enterpriseReplayStore"
import { stagger, variants } from "@/lib/motion-tokens"
import { Puzzle } from "lucide-react"

export default function ConnectorReplayPage() {
  const [collapsed, setCollapsed] = useState(false)
  const sidebarWidth = collapsed ? 60 : 172
  const loadConnectors = useEnterpriseReplayStore((s) => s.loadConnectors)
            ...
            <motion.div variants={variants.fadeUp} className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  <Puzzle className="h-6 w-6 text-[#38B88A]" />
                  <h1 className="text-[1.6rem] font-bold tracking-[-0.02em] text-[#111827]">Connector Activity</h1>
                </div>
                <p className="mt-0.5 text-[0.82rem] text-[#6B7280]">GitHub, Jira, Slack, Teams, ServiceNow, Confluence, Notion, Azure DevOps — calls, responses, failures, retries.</p>
              </div>
              <ExportButton executionId={executionId} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <ExecutionIdInput onLoad={(id) => loadConnectors(id)} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <ConnectorReplayPanel />
            </motion.div>
          </motion.div>
        </main>
      </div>
