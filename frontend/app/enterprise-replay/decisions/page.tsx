...
import { DecisionExplorerPanel } from "@/components/enterprise-replay/decision-explorer"
...
export default function DecisionExplorerPage() {
  ...
  const loadDecisions = useEnterpriseReplayStore((s) => s.loadDecisions)
            ...
            <motion.div variants={variants.fadeUp} className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  <Brain className="h-6 w-6 text-[#38B88A]" />
                  <h1 className="text-[1.6rem] font-bold tracking-[-0.02em] text-[#111827]">Decision Explorer</h1>
                </div>
                <p className="mt-0.5 text-[0.82rem] text-[#6B7280]">Reasoning chain, LLM requests, decision confidence, validation, and governance.</p>
              </div>
              <ExportButton executionId={executionId} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <ExecutionIdInput onLoad={(id) => loadDecisions(id)} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
