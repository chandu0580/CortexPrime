...
import { WorkerReplayPanel } from "@/components/enterprise-replay/worker-replay"
import { useEnterpriseReplayStore } from "@/store/enterpriseReplayStore"
import { stagger, variants } from "@/lib/motion-tokens"
import { Bot } from "lucide-react"

export default function WorkerReplayPage() {
  const [collapsed, setCollapsed] = useState(false)
  const sidebarWidth = collapsed ? 60 : 172
  const loadWorkers = useEnterpriseReplayStore((s) => s.loadWorkers)
            ...
            <motion.div variants={variants.fadeUp} className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  <Bot className="h-6 w-6 text-[#38B88A]" />
                  <h1 className="text-[1.6rem] font-bold tracking-[-0.02em] text-[#111827]">Worker Replay</h1>
                </div>
                <p className="mt-0.5 text-[0.82rem] text-[#6B7280]">Browser actions, voice events, desktop actions, and runtime state.</p>
              </div>
              <ExportButton executionId={executionId} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <ExecutionIdInput onLoad={(id) => loadWorkers(id)} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <WorkerReplayPanel />
            </motion.div>
