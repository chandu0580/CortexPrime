...
import { EventExplorerPanel } from "@/components/enterprise-replay/event-explorer"
...
export default function EventExplorerPage() {
  ...
  const loadEvents = useEnterpriseReplayStore((s) => s.loadEvents)
            ...
            <motion.div variants={variants.fadeUp} className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  <ListTodo className="h-6 w-6 text-[#38B88A]" />
                  <h1 className="text-[1.6rem] font-bold tracking-[-0.02em] text-[#111827]">Event Explorer</h1>
                </div>
                <p className="mt-0.5 text-[0.82rem] text-[#6B7280]">Every event published through EventBus — mission, worker, connector, approval, security, memory, graph events.</p>
              </div>
              <ExportButton executionId={executionId} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <ExecutionIdInput onLoad={(id) => loadEvents(id)} />
            </motion.div>
            <motion.div variants={variants.fadeUp}>
              <EventExplorerPanel />
            </motion.div>
