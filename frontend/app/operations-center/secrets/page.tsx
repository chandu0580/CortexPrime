...
import SecretsPanel from "@/components/operations-center/secrets-panel"
import { ChevronLeft } from "lucide-react"
import Link from "next/link"

export default function OpsSecretsPage() {
        ...
        <main className="flex-1 px-5 py-5">
          <motion.div initial="hidden" animate="visible" variants={stagger(0.04)} className="mx-auto max-w-[1500px]">
            <motion.div variants={variants.fadeUp} className="mb-4">
              ...
              <h1 className="text-[1.5rem] font-bold tracking-[-0.02em] text-[#111827] mt-2">Secrets</h1>
              <p className="text-[0.82rem] text-[#6B7280]">Manage vaults, Azure Key Vault, AWS Secrets Manager, rotation, validation, and expiry.</p>
            </motion.div>
            <motion.div variants={variants.fadeUp}><SecretsPanel /></motion.div>
          </motion.div>
        </main>
      </div>
    </div>
