...
import { ConfigurationWizardPanel } from "@/components/pilot-readiness/configuration-wizard-panel"
import { ChevronLeft, Settings } from "lucide-react"
import Link from "next/link"

export default function ConfigurationWizardPage() {
          ...
          <motion.div initial="hidden" animate="visible" variants={stagger(0.04)} className="mx-auto max-w-[1500px]">
            <motion.div variants={variants.fadeUp} className="mb-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Settings className="h-5 w-5 text-[#38B88A]" />
                <h1 className="text-[1.5rem] font-bold tracking-[-0.02em] text-[#111827]">Environment Configuration</h1>
              </div>
              <p className="text-[0.82rem] text-[#6B7280]">Configure organization, authentication, LLM providers, databases, connectors, and security settings.</p>
            </motion.div>
            <motion.div variants={variants.fadeUp}><ConfigurationWizardPanel /></motion.div>
          </motion.div>
        </main>
      </div>
    </div>
