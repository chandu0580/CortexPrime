...
import { DeploymentValidationPanel } from "@/components/pilot-readiness/deployment-validation-panel"
import { ChevronLeft, ShieldCheck } from "lucide-react"
import Link from "next/link"

export default function DeploymentValidationPage() {
          ...
          <motion.div initial="hidden" animate="visible" variants={stagger(0.04)} className="mx-auto max-w-[1500px]">
            <motion.div variants={variants.fadeUp} className="mb-4">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <h1 className="text-[1.5rem] font-bold tracking-[-0.02em] text-[#111827]">Deployment Validation</h1>
              </div>
              <p className="text-[0.82rem] text-[#6B7280]">Verify infrastructure, connectors, workers, runtime, memory, knowledge graph, health, and security.</p>
            </motion.div>
            <motion.div variants={variants.fadeUp}><DeploymentValidationPanel /></motion.div>
          </motion.div>
        </main>
      </div>
    </div>
