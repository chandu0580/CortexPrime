...
export default function MetricCard({
  ...
  const drawSparkline = (data: number[]) => {
    ...
    const strokeColor = trend === "down" 
      ? (isAlerts ? "#38B88A" : "#EF4444") // Green if alerts fall, red if uptime/availability falls
      : (isAlerts ? "#EF4444" : "#38B88A");
  ...
  // Trend behavior adjustments (active alerts going down is positive)
  const isAlerts = title.toLowerCase().includes("alerts");
  const isPositiveTrend = (isUp && !isAlerts) || (isDown && isAlerts);
