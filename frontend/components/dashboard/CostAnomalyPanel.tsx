...
import type { CostAnomalyCheck } from "@/services/dashboard/costAnomaly"
...
export function CostAnomalyPanel({
    ...
    return (
        <Card className="p-5">
            <SectionHead title="Cost Anomalies" action="View All" />
            <div className="space-y-3">
                ...
                {showEmpty && (
                    <div className="flex flex-col items-center gap-2 py-6 text-center">
                        <DollarSign className="h-8 w-8 text-[#D1D5DB]" />
                        <p className="text-[0.86rem] font-medium text-[#9CA3AF]">No spend checked yet</p>
                    ...
                    <>
                        {recent.map((c) => {
                            ...
                            const detail = isClean
                                ? `$${c.today_cost.toFixed(2)} today — normal`
