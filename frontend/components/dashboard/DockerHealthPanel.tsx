...
import type { DockerHealthCheck } from "@/services/dashboard/dockerHealth"
...
export function DockerHealthPanel({
    ...
    return (
        <Card className="p-5">
            <SectionHead title="Container Health" action="View All" />
            <div className="space-y-3">
                ...
                {showEmpty && (
                    <div className="flex flex-col items-center gap-2 py-6 text-center">
                        <Box className="h-8 w-8 text-[#D1D5DB]" />
                        <p className="text-[0.86rem] font-medium text-[#9CA3AF]">No containers checked yet</p>
                        <p className="text-[0.75rem] text-[#B0B7C3]">Polled continuously every 30s</p>
                    </div>
                )}
                {!showSkeleton && !showEmpty && (
                    <>
                        {recent.map((c) => {
                            ...
                            return (
                                <div key={c.container} className="flex items-center gap-3">
                                    <div
