...
import {
  ...
  Compass, Database, ExternalLink, FileText, Filter, Globe,
  Home, Lightbulb, Mic, Monitor, MoreVertical, Plus,
  Puzzle, RefreshCw, Search, Settings, Shield, ShieldCheck,
  Target, Zap,
} from "lucide-react";
  ...
  return (
    <header className="fixed top-0 right-0 z-30 flex items-center gap-3 border-b border-[#E8EDF3] bg-white/96 px-5 py-2.5 backdrop-blur transition-all duration-300"
      ...
      <label className="relative flex h-9 w-[200px] items-center">
        ...
        <input type="search" placeholder="Search research..." className="h-full w-full rounded-[12px] border border-[#E8EDF3] bg-[#F5F7FA] pl-9 pr-12 text-[0.8rem] text-[#111827] outline-none placeholder:text-[#9CA3AF] focus:border-[#B7E5D3]" />
...
function Sparkline({ data, color = "#38B88A" }: { data: number[]; color?: string }) {
  ...
  const gid = `sp-${color.replace("#", "")}-${Math.random().toString(36).slice(2, 6)}`;
  return (
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    running:   "bg-[#ECFBF4] text-[#2F9F77]",
    completed: "bg-[#DBEAFE] text-[#1D4ED8]",
    queued:    "bg-[#F3F4F6] text-[#6B7280]",
    searching: "bg-[#ECFBF4] text-[#2F9F77]",
    reading:   "bg-[#ECFBF4] text-[#2F9F77]",
    analyzing: "bg-[#ECFBF4] text-[#2F9F77]",
  };
  ...
  const labels: Record<string, string> = {
    running: "Running", completed: "Completed", queued: "Queued",
    searching: "Running", reading: "Running", analyzing: "Running",
  };
  const key = status.toLowerCase();
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[0.62rem] font-semibold", styles[key] || "bg-[#F3F4F6] text-[#6B7280]")}>
      <span className={cn("h-1.5 w-1.5 rounded-full", dotStyles[key] || "bg-[#9CA3AF]")} />
      {labels[key] || status}
    </span>
  );
}
...
function ProgressDonut() {
  const total = progressDistribution.reduce((a, s) => a + s.value, 0);
    ...
    <div className="flex items-center gap-5">
      <div className="relative flex-shrink-0">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-[1.4rem] font-bold text-[#111827]">18</span>
          <span className="text-[0.62rem] font-semibold text-[#9CA3AF]">Active</span>
        </div>
      </div>
...
function InsightsByCategoryChart() {
  const max = Math.max(...categoryInsights.map((c) => c.value));
  return (
      ...
      {categoryInsights.map((c) => (
        <div key={c.label} className="flex items-center gap-3">
          <div className="flex w-[70px] shrink-0 items-center gap-1.5">
            ...
            <span className="text-[0.72rem] font-semibold text-[#6B7280] truncate">{c.label}</span>
          </div>
          <div className="relative flex-1 h-2.5 rounded-full bg-[#F1F5F9]">
            <div className="h-full rounded-full" style={{ width: `${(c.value / max) * 100}%`, backgroundColor: c.color }} />
          </div>
          <span className="w-8 text-right text-[0.72rem] font-bold text-[#374151]">{c.value}</span>
        </div>
      ))}
    </div>
  );
...
export default function ResearchCenter() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return researchProjects;
    return researchProjects.filter((p) =>
      p.title.toLowerCase().includes(q) || p.summary.toLowerCase().includes(q) || p.agent.toLowerCase().includes(q)
    );
  }, [search]);

  const firstFiveMetrics = researchMetrics.slice(0, 5);
        ...
        <main className="flex-1 px-5 py-5">
          <motion.div initial="hidden" animate="visible" variants={stagger(0.04, 0.01)} className="mx-auto max-w-[1400px] space-y-5">
            ...
            <motion.div variants={variants.fadeUp} className="flex items-start justify-between gap-4">
              <div>
                <h1 className="text-[1.6rem] font-bold tracking-[-0.02em] text-[#111827]">Research Center</h1>
                <p className="mt-0.5 text-[0.82rem] text-[#6B7280]">Discover, analyze, and synthesize insights from the web, documents, and data sources.</p>
              </div>
                ...
                return (
                  <div key={m.label} className="rounded-[18px] border border-[#E8EDF3] bg-white p-4 shadow-[0_1px_8px_rgba(148,163,184,0.06)]">
                    ...
                    <div className="mt-2 flex items-baseline gap-2">
                      <span className="text-[1.55rem] font-bold tracking-tight text-[#111827]">{m.value}</span>
                      <span className="text-[0.72rem] font-bold text-[#38B88A]">{m.trend}</span>
                    </div>
                    <p className="text-[0.66rem] text-[#9CA3AF]">vs last 7 days</p>
            ...
            <motion.div variants={stagger(0.04)} className="grid gap-5 lg:grid-cols-12">
              ...
              <div className="rounded-[18px] border border-[#E8EDF3] bg-white shadow-[0_1px_8px_rgba(148,163,184,0.06)] lg:col-span-8">
                {/* Header */}
                <div className="flex flex-col gap-3 border-b border-[#E8EDF3] px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between">
                  <h2 className="text-[0.88rem] font-bold text-[#111827]">Research Projects</h2>
                  <div className="relative h-9 w-full sm:w-[240px]">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[720px] border-collapse text-left">
                    <thead>
                      <tr className="border-b border-[#E8EDF3] text-[0.65rem] font-bold uppercase tracking-wider text-[#9CA3AF]">
                    ...
                    <tbody>
                      {filtered.map((p, i) => {
                        ...
                        const barColor =
                          ...
                          p.status === "queued" ? "#CBD5E1" : "#38B88A";
                        return (
                          <tr key={i} className="border-b border-[#F1F5F9] last:border-0 hover:bg-[#F8FAF9]/60 cursor-pointer transition-colors">
                            {/* Project */}
                            <td className="px-5 py-3">
                              <div className="flex items-start gap-3 min-w-0">
                                <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-[8px] bg-[#F0FBF6] text-[#38B88A]">
                                  <Icon className="h-3.5 w-3.5" />
                                </div>
                                <div className="min-w-0">
                                  <p className="text-[0.78rem] font-bold text-[#111827] truncate max-w-[150px]">{p.title}</p>
                                  <p className="text-[0.66rem] text-[#9CA3AF] truncate max-w-[150px]">{p.summary}</p>
                                </div>
                              </div>
                            </td>
                            {/* Goal */}
                            <td className="px-3 py-3">
                              <p className="text-[0.72rem] text-[#4B5563] max-w-[140px] leading-snug">{p.goal}</p>
                            </td>
                            {/* Agent */}
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-1.5">
                                <div className="flex h-5 w-5 items-center justify-center rounded-full bg-[#ECFBF4] text-[#38B88A]">
                                  <Search className="h-2.5 w-2.5" />
                                </div>
                                <span className="text-[0.72rem] font-semibold text-[#374151] whitespace-nowrap">{p.agent}</span>
                              </div>
                            </td>
                            {/* Status */}
                            ...
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-2">
                                <div className="h-1.5 w-16 overflow-hidden rounded-full bg-[#E8EDF3]">
                                  <div className="h-full rounded-full" style={{ width: `${p.progress}%`, backgroundColor: barColor }} />
                                </div>
                                <span className="text-[0.68rem] font-bold text-[#374151]">{p.progress}%</span>
                              </div>
                            </td>
                            {/* Updated */}
                            <td className="px-3 py-3 text-[0.72rem] text-[#9CA3AF] whitespace-nowrap">{p.updated}</td>
                            {/* Actions */}
                ...
                {/* Pagination */}
                <div className="flex flex-col gap-3 border-t border-[#E8EDF3] px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-[0.72rem] text-[#6B7280]">Showing 1 to 7 of 18 results</p>
                  <div className="flex items-center gap-1">
                    <button className="flex h-7 w-7 items-center justify-center rounded-[8px] border border-[#E8EDF3] text-[#6B7280] hover:bg-[#F5F7FA]">
                      <ChevronLeft className="h-3.5 w-3.5" />
                    </button>
                    {[1, 2, 3].map((p) => (
                      <button key={p} className={cn(
                        "flex h-7 w-7 items-center justify-center rounded-[8px] border text-[0.72rem] font-semibold",
                        p === 1 ? "border-[#38B88A] bg-[#38B88A] text-white" : "border-[#E8EDF3] text-[#374151] hover:bg-[#F5F7FA]"
                      )}>{p}</button>
                    ))}
                    <button className="flex h-7 items-center justify-center gap-1 rounded-[8px] border border-[#E8EDF3] px-2 text-[0.72rem] font-bold text-[#374151] hover:bg-[#F5F7FA]">
                      <ChevronRight className="h-3.5 w-3.5" />
                    </button>
                    <div className="ml-1 flex h-7 items-center gap-1 rounded-[8px] border border-[#E8EDF3] px-2 text-[0.72rem] font-semibold text-[#374151] cursor-pointer hover:bg-[#F5F7FA]">
                      10 / page <ChevronDown className="h-2.5 w-2.5 text-[#9CA3AF]" />
                    </div>
                  </div>
                </div>
              </div>

              {/* RIGHT: Research Agents + Recent Activity */}
              <div className="flex flex-col gap-4 lg:col-span-4">

                {/* Research Agents */}
                <div className="rounded-[18px] border border-[#E8EDF3] bg-white p-4 shadow-[0_1px_8px_rgba(148,163,184,0.06)]">
                  <div className="mb-3 flex items-center justify-between">
                    <h3 className="text-[0.85rem] font-bold text-[#111827]">Research Agents</h3>
                    <button className="text-[0.72rem] font-bold text-[#38B88A] hover:underline">View All</button>
                  </div>
                  <div className="space-y-2.5">
                    {researchAgents.map((a, i) => {
                      const Icon = a.icon;
                      return (
                        <div key={i} className="flex items-center gap-3 rounded-[10px] p-2 hover:bg-[#F8FAF9] transition-colors cursor-pointer">
                          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[8px] bg-[#F0FBF6] text-[#38B88A]">
                            <Icon className="h-3.5 w-3.5" />
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="text-[0.76rem] font-bold text-[#111827]">{a.name}</p>
                            <p className="text-[0.66rem] text-[#9CA3AF] truncate">{a.specialty}</p>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className="inline-flex items-center gap-1 rounded-full bg-[#ECFBF4] px-1.5 py-0.5 text-[0.62rem] font-bold text-[#2F9F77]">
                              <span className="h-1 w-1 rounded-full bg-[#38B88A]" />Active
                            </span>
                            <span className="text-[0.72rem] font-bold text-[#374151]">{a.tasks}</span>
                          </div>
                        </div>
                      );
                    })}
                    ...
                    {recentActivity.map((a, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-start gap-2.5">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-[0.72rem] font-semibold text-[#111827]">{a.label}</p>
                          </div>
            ...
            <motion.div variants={stagger(0.04)} className="grid gap-5 lg:grid-cols-12">
              ...
              <div className="rounded-[18px] border border-[#E8EDF3] bg-white p-5 shadow-[0_1px_8px_rgba(148,163,184,0.06)] lg:col-span-4">
                <h3 className="mb-4 text-[0.88rem] font-bold text-[#111827]">Research Progress Overview</h3>
                <ProgressDonut />
              </div>
              ...
              <div className="rounded-[18px] border border-[#E8EDF3] bg-white p-5 shadow-[0_1px_8px_rgba(148,163,184,0.06)] lg:col-span-4">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-[0.88rem] font-bold text-[#111827]">Insights by Category</h3>
                  <button className="text-[0.72rem] font-bold text-[#38B88A] hover:underline">View Details</button>
                </div>
                <InsightsByCategoryChart />
                {/* X axis labels */}
                <div className="mt-3 flex justify-between text-[0.62rem] text-[#9CA3AF]">
              ...
              {/* Top Research Topics */}
              <div className="rounded-[18px] border border-[#E8EDF3] bg-white p-5 shadow-[0_1px_8px_rgba(148,163,184,0.06)] lg:col-span-4">
                <h3 className="mb-4 text-[0.88rem] font-bold text-[#111827]">Top Research Topics</h3>
                <div className="grid grid-cols-2 gap-2">
                  {topTopics.map((t, i) => (
                    <div key={i} className="flex items-center justify-between rounded-[8px] bg-[#F5F7FA] px-2.5 py-1.5">
                      <span className="text-[0.72rem] font-semibold text-[#374151] truncate">{t.label}</span>
                      <span className="ml-2 shrink-0 text-[0.7rem] font-bold text-[#9CA3AF]">{t.count}</span>
                    </div>
                  ))}
                </div>
                <button className="mt-3 w-full rounded-[10px] border border-[#E8EDF3] py-1.5 text-[0.72rem] font-bold text-[#374151] hover:bg-[#F5F7FA]">
                  View All Topics
                </button>
              </div>
