...
export default function ExecutiveMetricCard({
  ...
  const drawSparkline = (data: number[]) => {
    ...
    // Choose color based on trend/status
    let strokeColor = "#38B88A"; // Green
    if (trend === "down") {
      strokeColor = status === "success" ? "#38B88A" : "#EF4444"; // cost down is success, so green; else red
  ...
  // Custom behavior: if it's "Cost Today" and cost goes down, it's positive. Let's make cost trending down green!
  const isCost = title.toLowerCase().includes("cost");
  const displaysPositiveTrend = (isUp && !isCost) || (isDown && isCost);

  const finalTrendColor = displaysPositiveTrend
    ? "text-[#2F9F77] bg-[#E8F5EE]"
      ...
      <div className="mt-4 flex items-baseline justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center gap-1.5">
            <span className={cn("inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-bold", finalTrendColor)}>
