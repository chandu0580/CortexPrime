interface ApiCallRecord { method: string; path: string; status: number; latencyMs: number; timestamp: string }
interface FailureRecord { method: string; path: string; error: string; timestamp: string }
const apiCalls: ApiCallRecord[] = []; const failures: FailureRecord[] = []

export const AzureDevOpsTelemetry = {
  ...
  async getMetrics(): Promise<{ totalCalls: number; totalFailures: number; successRate: number; averageLatencyMs: number }> {
    const totalCalls = apiCalls.length; const totalFailures = failures.length
