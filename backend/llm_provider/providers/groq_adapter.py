...
class GroqAdapter(LLMProvider):
    def __init__(self) -> None:
        ...
        self._default_model = os.getenv("MODEL_GROQ", "llama-3.3-70b-versatile")
        ...
        default_models = [
            LLMModelInfo(
                ...
                version="llama-3.3-70b-versatile", is_default=True,
            ),
            LLMModelInfo(
            ...
            LLMModelInfo(
                name="qwen-2.5-32b", provider="groq",
                capabilities={ModelCapability.CHAT, ModelCapability.STREAMING,
                              ModelCapability.FUNCTION_CALLING},
                context_window=32768, max_output_tokens=8192,
                cost_per_1k_input=0.0, cost_per_1k_output=0.0,
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if api_key:
                ...
                log.info("GroqAdapter initialized")
                return True
            log.warning("No GROQ_API_KEY configured")
            return False
        except Exception as exc:
            log.warning("GroqAdapter initialization failed: %s", exc)
            return False
    ...
    async def stream(self, request: LLMRequest) -> AsyncGenerator[StreamChunk, None]:
        try:
            if not self._client:
                yield StreamChunk(done=True, error="Groq client not configured")
                return
            model = request.model or self._default_model
            messages = self._build_messages(request)
            ...
            log.warning("Groq stream error: %s", exc)
            yield StreamChunk(
