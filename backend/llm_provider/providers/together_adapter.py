...
class TogetherAdapter(LLMProvider):
    def __init__(self) -> None:
        self._client: Optional[AsyncOpenAI] = None
        self._initialized = False
        self._default_model = os.getenv("MODEL_TOGETHER", "meta-llama/Llama-3.3-70B-Instruct-Turbo")
        ...
        default_models = [
            LLMModelInfo(
                ...
                cost_per_1k_input=0.88, cost_per_1k_output=0.88,
                version="llama-3.3-70b-turbo", is_default=True,
            ),
            LLMModelInfo(
                name="mistralai/Mixtral-8x22B-Instruct-v0.1", provider="together",
                capabilities={ModelCapability.CHAT, ModelCapability.STREAMING,
                              ModelCapability.FUNCTION_CALLING},
                context_window=65536, max_output_tokens=4096,
                cost_per_1k_input=0.90, cost_per_1k_output=0.90,
                version="mixtral-8x22b-v0.1",
            ),
            LLMModelInfo(
                name="google/gemma-2-27b-it", provider="together",
                capabilities={ModelCapability.CHAT, ModelCapability.STREAMING},
                context_window=8192, max_output_tokens=4096,
                cost_per_1k_input=0.27, cost_per_1k_output=0.27,
                version="gemma-2-27b-it",
            ),
            LLMModelInfo(
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if api_key:
                ...
                log.info("TogetherAdapter initialized")
                return True
            log.warning("No TOGETHER_API_KEY configured")
            return False
        except Exception as exc:
            log.warning("TogetherAdapter initialization failed: %s", exc)
            return False
        ...
        try:
            if not self._client:
                return LLMResponse(
                    ...
                    error="Together client not configured",
    ...
    async def stream(self, request: LLMRequest) -> AsyncGenerator[StreamChunk, None]:
        try:
            if not self._client:
                yield StreamChunk(done=True, error="Together client not configured")
                return
            model = request.model or self._default_model
            messages = self._build_messages(request)
            ...
            log.warning("Together stream error: %s", exc)
            yield StreamChunk(
    ...
    async def embed(self, request: EmbeddingRequest) -> EmbeddingResult:
        return EmbeddingResult(
            error="Together does not support embeddings in this adapter",
