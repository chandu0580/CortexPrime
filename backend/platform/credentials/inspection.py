"""Structured, field-aware secret inspection — a platform primitive.

Detects whether credential material entered a value bound for anywhere it must
not be: model context, a trace, an audit payload, a durable execution spec, or
a World Plane observation. Lives in the platform layer (Phase 7.2) so both the
harness firewall and the World Plane ingestion boundary can use it without
importing each other — ``backend.harness.firewall`` re-exports it unchanged.
