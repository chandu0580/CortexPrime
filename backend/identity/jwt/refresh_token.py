...
class RefreshTokenProvider:
    ...
    async def validate(self, token: str) -> Optional[TokenClaims]:
        try:
            ...
            kid = headers.get("kid", "")
            if not kid:
                log.debug("Refresh token missing kid header")
                return None
            ...
            if payload.get("type") != "refresh":
                log.debug("Token type is not refresh: %s", payload.get("type"))
                return None
            ...
            log.debug("Refresh token validation failed: %s", exc)
            return None
        except Exception as exc:
            log.error("Refresh token validation error: %s", exc)
            return None
